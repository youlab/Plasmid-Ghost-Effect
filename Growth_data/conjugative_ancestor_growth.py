...
# plasmid burden = max growth rate(DA28102) / max growth rate(plasmid) - 1
# Here we use mean max-growth-rate across replicates and propagate SEM for the ratio:
...
ratio = da_mean / summary["mean_growth_rate"]
summary["burden"] = ratio - 1.0

rel_var = (summary["sem_growth_rate"] / summary["mean_growth_rate"]) ** 2 + (da_sem / da_mean) ** 2
summary["burden_sem"] = ratio * np.sqrt(rel_var)
summary.loc[summary.index == "DA28102", "burden_sem"] = 0.0

summary.to_excel("./processed_data/conjugative_plasmid_burden_ancestor.xlsx")
...
for j, plasmid in enumerate(order):
    ...
    ax_gr.scatter(x_j, y_vals, s=100, linewidths=1, facecolor=colors[j], edgecolors="k", zorder=3)
...
fig_gr.savefig("./figures/conjugative_growth_rate_ancestor.svg", dpi=300)
plt.show()