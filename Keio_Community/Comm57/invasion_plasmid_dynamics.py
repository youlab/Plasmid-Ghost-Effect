...
def visualize_dynamics(ax,time,data,data_se,color,zorder=0):
    zero_idx = (data < 0.1)
    data[zero_idx] = 0.1
    bio_rep = 3  # number of biological replicates

    for k in range(bio_rep):
        ax.plot(time, data[k, :], color=color, linewidth=1.5, zorder=zorder)
        ax.scatter(time, data[k, :], s=60, color=color, linewidth=1, edgecolors='black', zorder=zorder)
        ax.errorbar(time, data[k, :], data_se[k, :], marker='None', linewidth=0, elinewidth=1,
...
Plasmids = ["pCU1","R6K"]
Abs = ["Carb","Strp"]
yloc = {"pCU1":0.85,"R6K":0.4}
time = np.array([0, 2, 3, 7, 10, 15, 20])
time_inv = np.array([0, 2, 5, 8, 10, 15]) + 3
for i, plasmid in enumerate(Plasmids):
    # Comm57 + plasmid
    fig,ax=plt.subplots(1,1,figsize=(2.6, 2.3))
    ...
    inv_abundance = np.load(f"./processed_data/{plasmid}_inv_mean.npy")
    inv_se_abundance = np.load(f"./processed_data/{plasmid}_inv_se.npy")
    ...
    zero_idx = (inv_abundance < 0.1)
    inv_abundance[zero_idx] = 0.1

    n_ab = abundance.shape[0]  # with / without antibiotic pulse
    bio_rep = abundance.shape[1]  # number of biological replicates
    
    ax = visualize_dynamics(ax,time,abundance[1],se_abundance[1],colors[1])
    ax = visualize_dynamics(ax,time_inv,inv_abundance,inv_se_abundance,colors[2],zorder=-3)
    ax.fill_between(x=[2, 3], y1=[1, 1], y2=[150, 150], color="#808080", zorder=-20, alpha=0.3)
    ...
    ax.set_yscale("log")
    fig.subplots_adjust(left=0.3, right=0.95, bottom=0.23, top=0.85)
    # fig.savefig(f"./figures/Comm57_{plasmid}_dynamics_{dilution}dilution_winvasion.png",dpi=300)
    # fig.savefig(f"./figures/Comm57_{plasmid}_dynamics_{dilution}dilution_winvasion.svg")

    fig2, ax2 = plt.subplots(1, 1, figsize=(2.0, 2.2))
    HL = np.zeros((3, bio_rep))
    SE_HL = np.zeros((3, bio_rep))
    ...
    for k in range(bio_rep):
        ...
        SE_HL[1, k] = t_se
    # For invasion group: calculate half-life from day 3
    for k in range(bio_rep):
    ...
    pd.DataFrame(records).to_csv(f"./processed_data/{plasmid}_{dilution}_halflife_winvasion.csv", index=False)

    print(f"\nComm57+{plasmid} Welch's t-test")
    p_value1, report1 = report_welch_t(HL[1,:], HL[0,:], group_names=(f"+{Abs[i]}","LB"))
    print(report1)
    ...
    ax2.text(0.5, np.max(HL) * 1.11, p_to_star(p_value1), ha='center', va='bottom', fontsize=12)
    ax2.plot([0,1], np.ones(2) * np.max(HL) * 1.16, c="k", lw=1)
    ...
    # ax2.set_ylabel(r"$\tau_{1/2}$ (days)")
    fig2.subplots_adjust(left=0.25, right=0.95, bottom=0.3, top=0.95)
    # fig2.savefig(f"./figures/Comm57_{plasmid}_half_life_{dilution}dilution_winvasion.png",dpi=300)
    # fig2.savefig(f"./figures/Comm57_{plasmid}_half_life_{dilution}dilution_winvasion.svg")
plt.show()
