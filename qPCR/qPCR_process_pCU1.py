# processing qPCR data for DA28102 + pCU1
...
# Calibration data
file = "./Raw_data/0507_pCU1_all.xlsx"
...
pCU1_fam = np.zeros((8, 6)) * np.nan
pCU1_vic = np.zeros((8, 6)) * np.nan
...
for i in range(6):
    ax.scatter(log2_dilution, pCU1_fam[:,i], color=colors[0], marker="o", edgecolor=colors[0], facecolor="None", s=50, lw=1.5)
    ax.scatter(log2_dilution, pCU1_vic[:,i], color=colors[1], marker="o", edgecolor=colors[1], facecolor="None", s=50, lw=1.5)   

# Weighted fit 
x = np.tile(log2_dilution, 6)  # repeat dilution for each replicate
popt_fam, pcov_fam, p_fam, R2_fam, x_band_fam, y_band_fam, ci_y_fam = linear_fit(x, pCU1_fam.T.flatten())
popt_vic, pcov_vic, p_vic, R2_vic, x_band_vic, y_band_vic, ci_y_vic = linear_fit(x, pCU1_vic.T.flatten())
...
np.savetxt(
    "./processed_data/DA28102-pCU1-calibration.txt",
    slope_summary,
    fmt="%.6f",
    delimiter="\t",
)
...
ax.set_ylabel("Ct")
ax.set_title("DA28102+pCU1")
fig.tight_layout()
fig.savefig("./figures/DA28102-pCU1-calibration.png", dpi=300)
fig.savefig("./figures/DA28102-pCU1-calibration.svg", dpi=300)
# measurement data
file = "./Raw_data/0507_pCU1_all.xlsx"
...
# MG1655 + pCU1
pCU1_fam = np.zeros((3, 3)) * np.nan
pCU1_vic = np.zeros((3, 3)) * np.nan

for i, row in enumerate("DEF"):
...
CN = 2**(pCU1_vic/popt_vic[0]) / 2**(pCU1_fam/popt_fam[0])
np.savetxt("./processed_data/pCU1_CN.txt",CN)