...
colors = ["#AAAAAA", "#23A249", "#B53030", "#74B3EB", "#FF9000"]
...
od_files = {
    "MG1655": "./processed_data/MG1655_ancestor_LB_OD.xlsx",
    "pSC101": "./processed_data/pSC101_ancestor_LB_OD.xlsx",
    "colE1": "./processed_data/colE1_ancestor_LB_OD.xlsx",
    "pUC": "./processed_data/pUC_ancestor_LB_OD.xlsx",
}
...
df_gr = pd.concat(
    [
        growth_rate_from_replicates(time_mg, mg_od, mg_reps, "MG1655"),
        growth_rate_from_replicates(time_mg, psc101_od, psc101_reps, "pSC101"),
        growth_rate_from_replicates(time_mg, cole1_od, cole1_reps, "colE1"),
        growth_rate_from_replicates(time_mg, puc_od, puc_reps, "pUC"),
    ],
    ignore_index=True,
)
df_gr.to_excel("./processed_data/GFP_growth_rates_ancestor.xlsx", index=False)

# mean +/- sem summary bar plot
order = ["MG1655", "pSC101", "colE1", "pUC"]
...
# plasmid burden = max growth rate(plasmid) / max growth rate(MG1655)
...
mg_mean = np.array(summary.loc["MG1655", "mean_growth_rate"], dtype=float).item()
mg_sem = np.array(summary.loc["MG1655", "sem_growth_rate"], dtype=float).item()

summary["burden"] = summary["mean_growth_rate"] / mg_mean

rel_var = (summary["sem_growth_rate"] / summary["mean_growth_rate"]) ** 2 + (mg_sem / mg_mean) ** 2
summary["burden_sem"] = summary["burden"] * np.sqrt(rel_var)
#summary.loc[summary.index == "MG1655", "burden_sem"] = 1

summary.to_excel("./processed_data/GFP_plasmid_burden_ancestor.xlsx", index=False)
...
fig_gr.tight_layout()
fig_gr.savefig("./figures/GFP_growth_rate_ancestor.png", dpi=300)
