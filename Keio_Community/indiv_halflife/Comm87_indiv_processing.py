"""
single Keio strain plasmid dynamics, for Comm87, focus on strains #1, #15, and #76 (R388)
------------------------------------------------
• Requires:  pandas, numpy, scipy
• Input:     LT24_KeioV2_indiv_plating.xlsx  (sheets “Day0”, “Day3”, …, “Day25”)
• Output:    1+R388_mean.npy / 1+R388_std.npy / 15+R388_mean.npy / 15+R388_std.npy / 76+R388_mean.npy / 76+R388_std.npy
"""
...
# ---------------------------------------------------------------------
excel_file = "./raw_data/LT24_KeioV2_indiv_plating.xlsx"
for t_idx, t in enumerate(days):
    ...
    for l in range(L):                         # host condition
...
# ---------------------------------------------------------------------
# 3. save one *_mean.npy and *_std.npy per host
# ---------------------------------------------------------------------
for l, strain in enumerate(Strains):
    abundance = Indiv_mean_P[l, :, :, :]         # shape (inits_condits, bio_rep, T)
    se       = Indiv_se_P[l, :, :, :]

    np.save(f"./processed_data/{strain}+R388_mean.npy",abundance)
    np.save(f"./processed_data/{strain}+R388_std.npy", se)
