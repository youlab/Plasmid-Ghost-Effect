...
file = "20260323_GE_DA_Ancestor_burden"
time = pd.read_excel(f"./raw_data/{file}.xlsx", sheet_name="Sheet2", header=None, usecols="B:KC", skiprows=47, nrows=1)
...
header_wells = pd.read_excel(f"./raw_data/{file}.xlsx", sheet_name="Sheet2",header=None, usecols="A", skiprows=49, nrows=96)
well_list = header_wells.iloc[:, 0].astype(str).tolist()

# load data
df_OD = pd.read_excel(f"./raw_data/{file}.xlsx", sheet_name="Sheet2", header=None, usecols="B:KC", skiprows=49, nrows=96).to_numpy(dtype=float)
...
df_timeseries_od.to_excel(f"./processed_data/DA28102_ancestor_LB+Cm_OD.xlsx", index=False)
...
df_timeseries_od = pd.DataFrame(OD_dict)
df_timeseries_od.to_excel(f"./processed_data/MG1655_ancestor_LB_OD.xlsx", index=False)
...
df_timeseries_od = pd.DataFrame(OD_dict)
df_timeseries_od.to_excel(f"./processed_data/pCU1_ancestor_LB+Cm+Carb_OD.xlsx", index=False)
...
df_timeseries_od = pd.DataFrame(OD_dict)
df_timeseries_od.to_excel(f"./processed_data/R6K_ancestor_LB+Cm+Carb_OD.xlsx", index=False)
plt.show()