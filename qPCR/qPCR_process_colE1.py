# processing qPCR data for MG1655 + colE1
...
# Calibration data
file = "./Raw_data/qPCR_GFP_ancestor_plate1.xlsx"
...
colE1_fam = np.zeros((8, 6)) * np.nan
colE1_vic = np.zeros((8, 6)) * np.nan

for i, row in enumerate("ABCDEFGH"):
    for j, col in enumerate(range(7, 13)):
        well = f"{row}{col}"
        ...
        if vic_Ct == "Undetermined":
            vic_Ct = np.nan
        colE1_fam[i, j] = fam_Ct
        colE1_vic[i, j] = vic_Ct

# plot calibration curve
fig,ax = plt.subplots(figsize=(3, 3))
for i in range(6):
    ax.scatter(log2_dilution, colE1_fam[:,i], color=colors[0], marker="o", edgecolor=colors[0], facecolor="None", s=50, lw=1.5)
    ax.scatter(log2_dilution, colE1_vic[:,i], color=colors[1], marker="o", edgecolor=colors[1], facecolor="None", s=50, lw=1.5)   

# Weighted fit 
x = np.tile(log2_dilution, 6)  # repeat dilution for each replicate
popt_fam, pcov_fam, p_fam, R2_fam, x_band_fam, y_band_fam, ci_y_fam = linear_fit(x, colE1_fam.T.flatten())
popt_vic, pcov_vic, p_vic, R2_vic, x_band_vic, y_band_vic, ci_y_vic = linear_fit(x, colE1_vic.T.flatten())
...
np.savetxt(
    "./processed_data/MG1655-colE1-calibration.txt",
    slope_summary,
    fmt="%.6f",
    delimiter="\t",
)
...
ax.set_ylabel("Ct")
ax.set_title("MG1655+colE1")
fig.tight_layout()
fig.savefig("./figures/MG1655-colE1-calibration.png", dpi=300)
fig.savefig("./figures/MG1655-colE1-calibration.svg", dpi=300)
# measurement data
...
# MG1655 + colE1
colE1_fam = np.zeros((3, 3)) * np.nan
colE1_vic = np.zeros((3, 3)) * np.nan

for i, row in enumerate("ABC"):
    for j, col in enumerate(range(10, 13)):
        well = f"{row}{col}"
        fam_Ct = df_fam.loc[df_fam["Well Position"] == well, "CT"].values[0]
        vic_Ct = df_vic.loc[df_vic["Well Position"] == well, "CT"].values[0]
        if fam_Ct == "Undetermined":
...
CN = 2**(colE1_vic/popt_vic[0]) / 2**(colE1_fam/popt_fam[0])
np.savetxt("./processed_data/colE1_CN.txt",CN)