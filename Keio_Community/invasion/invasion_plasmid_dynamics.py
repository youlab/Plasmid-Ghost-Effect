...
def visualize_dynamics(ax,time,data,data_se,color):
    zero_idx = (data < 0.1)
    data[zero_idx] = 0.1
    bio_rep = 3  # number of biological replicates

    for k in range(bio_rep):
        ax.plot(time, data[k, :], color=color, linewidth=1.5, zorder=-1)
        ax.scatter(time, data[k, :], s=60, color=color, linewidth=1, edgecolors='black', zorder=-1)
        ax.errorbar(time, data[k, :], data_se[k, :], marker='None', linewidth=0, elinewidth=1,
                    capsize=2, color="k", zorder=-1)
...
for i, plasmid in enumerate(Plasmids):
    if plasmid == "R388":
        ...
        post_Ab_se_abundance = np.load(f"./processed_data/post_antibiotic/{plasmid}_se.npy")[1]
    else:
        post_Ab_abundance = np.load(f"./processed_data/post_antibiotic/{plasmid}_100_mean.npy")[1]
        post_Ab_se_abundance = np.load(f"./processed_data/post_antibiotic/{plasmid}_100_se.npy")[1]
    ...
    time = np.array([0, 2, 3, 7, 10, 15, 20]) if plasmid!="R388" else np.array([0, 2, 3, 7, 10, 15, 20,25, 30, 36])

    fig,ax=plt.subplots(1,1,figsize=(2.05, 1.9))
    ax = visualize_dynamics(ax,time,post_Ab_abundance,post_Ab_se_abundance,colors[0]) # index 1 == after antibiotic pulse
    ...
    time_inv = np.array([0, 2, 5, 8, 10, 15])
    if plasmid == "R388":
        time_inv += 12
    else:
        time_inv += 3

    ax = visualize_dynamics(ax,time_inv,inv_abundance,inv_se_abundance,colors[1])
    ...
    fig.savefig(f"./figures/{plasmid}_invasion_dynamics.svg")
    ...
    t_start = 2
    for k in range(bio_rep):
        t, t_se = loglinear_crossing_time(time[t_start:], post_Ab_abundance[k, t_start:], post_Ab_se_abundance[k, t_start:],
                                          P0 / 2)
        HL[0, k] = t - 3  # start counting after the antibiotic pulse
        SE_HL[0, k] = t_se
    # For invasion group
    for k in range(bio_rep):
    ...
    print(f"{plasmid} +Ab Group half-life: {means[0]:.2f}±{sems[0]:.2f} days; "
          f"invaded Group half-life: {means[1]:.1f}±{sems[1]:.1f} days")
