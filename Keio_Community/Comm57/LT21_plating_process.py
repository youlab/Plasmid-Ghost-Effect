"""
Keio V3 community plasmid dynamics
------------------------------------------------
• Requires:  pandas, numpy, scipy
• Input:     GE_LT20_Keio.xlsx  (sheets “Day0”, “Day3”, …, “Day25”)
• Output:    R388_mean.npy / R388_std.npy / RP4 / pCU1 / R6K
"""
...
Indiv_mean_P = np.full((L,Ab_condits, bio_rep, T), np.nan)
Indiv_se_P  = np.full_like(Indiv_mean_P, np.nan)

# ---------------------------------------------------------------------
# 2. loop over sheets (“Day0”, “Day2”, …) and fill the arrays
# ---------------------------------------------------------------------
excel_file = "./raw_data/GE_LT21_Keio.xlsx"
for t_idx, t in enumerate(days):
    # read columns D:I
    # pandas keeps NaNs as float('nan') automatically
    df  = pd.read_excel(excel_file, sheet_name=f"Day{t}", usecols="D:I", skiprows=0, nrows=24)
    ...
    for l in range(L):                         # plasmid condition
        for ab in range(Ab_condits):        # Ab pulsed vs no pulse
            for br in range(bio_rep):          # biological replicate
                ...
                row_idx = (bio_rep * Ab_condits * l) + (bio_rep * ab) + br
                ...
                Indiv_mean_P[l,ab, br, t_idx] = mean_P
                Indiv_se_P[l,ab, br, t_idx] = se_P
...
for l, plasmid in enumerate(Plasmids):
    ...
    np.save(f"./processed_data/{plasmid}_100_mean.npy",abundance)
    np.save(f"./processed_data/{plasmid}_100_se.npy", se)
...
for t_idx, t in enumerate(days):
    # read columns D:I
    # pandas keeps NaNs as float('nan') automatically
    df  = pd.read_excel(excel_file, sheet_name=f"Day{t}", usecols="D:I", skiprows=25, nrows=24)
    data = df.to_numpy(dtype=float)          # shape (6, 24)
    for l in range(L):                         # plasmid condition
        for ab in range(Ab_condits):        # Ab pulsed vs no pulse
            for br in range(bio_rep):          # biological replicate
...
for l, plasmid in enumerate(Plasmids):
    ...
    np.save(f"./processed_data/{plasmid}_500_mean.npy",abundance)
    np.save(f"./processed_data/{plasmid}_500_se.npy", se)
