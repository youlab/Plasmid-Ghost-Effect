...
sample = "20260510_colE1_Day3_burden"
...
# MG1655 + colE1 at Day 3
for br in range(3):
    target_wells = []
    labels = []
    for colony in range(3):
        idx = br * 3 + colony + 2
        if br == 1 and colony == 2: # bio rep 1 colony 3, technical replicate 3 (D7) has outlier OD values, so exclude it from downstream analysis
            print(f"Excluding outlier: BioRep{br+1} Colony{colony+1} TechnicalRep3 (well D{idx})")
            target_wells += [f"{r}{idx}" for r in ["B", "C"]]  # only include 2 technical replicates for this colony
            labels += [f"c{colony+1}_{i}" for i in range(1,3,1)]
        else:
    ...
    df_timeseries_od.to_excel(f"./processed_data/Day3/colE1+BioRep{br+1}_OD.xlsx", index=False)

# MG1655 (colE1 cured) at Day 3
for br in range(3):
    target_wells = []
    labels = []
    for colony in range(3):
