# processing qPCR data for MG1655 + pUC
...
pUC_fam = np.zeros((8, 6)) * np.nan
pUC_vic = np.zeros((8, 6)) * np.nan
...
for i in range(6):
    ax.scatter(log2_dilution, pUC_fam[:,i], color=colors[0], marker="o", edgecolor=colors[0], facecolor="None", s=50, lw=1.5)
    ax.scatter(log2_dilution, pUC_vic[:,i], color=colors[1], marker="o", edgecolor=colors[1], facecolor="None", s=50, lw=1.5)   

# Weighted fit 
x = np.tile(log2_dilution, 6)  # repeat dilution for each replicate
popt_fam, pcov_fam, p_fam, R2_fam, x_band_fam, y_band_fam, ci_y_fam = linear_fit(x, pUC_fam.T.flatten())
popt_vic, pcov_vic, p_vic, R2_vic, x_band_vic, y_band_vic, ci_y_vic = linear_fit(x, pUC_vic.T.flatten())
...
np.savetxt(
    "./processed_data/MG1655-pUC-calibration.txt",
    slope_summary,
    fmt="%.6f",
    delimiter="\t",
)
...
# MG1655 + pUC
pUC_fam = np.zeros((3, 3)) * np.nan # biological replicates * technical replicates
pUC_vic = np.zeros((3, 3)) * np.nan
...
CN = 2**(pUC_vic/popt_vic[0]) / 2**(pUC_fam/popt_fam[0])
np.savetxt("./processed_data/pUC_CN.txt",CN)
